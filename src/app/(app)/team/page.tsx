  ...
  const Row = ({ p }: { p: (typeof all)[number] }) => (
    <li className="flex items-start gap-3 px-5 py-3">
      ...
      <div className="flex flex-col items-end gap-2">
        ...
        {p.id !== u.id && (director || p.role === "SITE_SUPERVISOR") && <UserAdmin id={p.id} name={p.name} active={p.active} canDeactivate={director} canReset />}
      </div>
    </li>
  );
  return (
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
        ...
        <Card className="h-fit p-5"><h2 className="mb-1 text-[15px] font-semibold">Add a team member</h2><p className="mb-4 text-xs text-muted">Add them to projects afterwards from a project&apos;s Team tab, or by assigning a task.</p><NewUserForm director={director} /></Card>
      </div>
    </>
  );
}
