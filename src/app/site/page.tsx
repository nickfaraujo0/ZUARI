...
export default async function SiteHome() {
  ...
  const [projects, tasks, unread] = await Promise.all([
    prisma.project.findMany({ where: projectScope(u), include: { photos: { orderBy: { takenAt: "desc" }, take: 1, select: { id: true } } }, orderBy: { createdAt: "asc" } }),
  ...
  return (
    <>
      ...
      <div className="px-5 pt-5">
        <h1 className="font-serif text-4xl font-semibold leading-tight text-river-deep">{greeting()}, {u.name.split(" ")[0]}</h1>
        ...
        {projects.map((p) => (
          <div key={p.id} className="relative overflow-hidden rounded-2xl bg-river text-ivory shadow-card">
            ...
            <div className="relative p-5 pt-14">
              ...
              <div className="mt-3 flex items-center gap-3"><div className="h-2 flex-1 overflow-hidden rounded-full bg-ivory/25"><div className="h-full rounded-full bg-sand" style={{ width: `${p.progress}%` }} /></div><span className="text-sm font-semibold">{p.progress}% complete</span></div>
            </div>
          </div>
        ))}
      </section>
      ...
      <section className="mt-5 grid grid-cols-3 gap-3 px-5">
        {[["Tasks", tasks.length], ["Completed", done], ["Remaining", open.length]].map(([l, v]) => (
          <div key={l} className="rounded-2xl border border-line bg-white p-3.5"><p className="text-3xl font-semibold leading-none tabular-nums">{v}</p><p className="mt-1.5 text-xs text-muted">{l}</p></div>
        ))}
      </section>

      <section className="mt-7 px-5">
        <h2 className="mb-3 text-xs font-medium uppercase tracking-[0.18em] text-muted">Quick actions</h2>
        <div className="grid grid-cols-3 gap-3">
          {qa.map((a) => (
            <Link key={a.href} href={a.href} className={`flex h-28 flex-col items-center justify-center gap-2.5 rounded-2xl border text-center text-sm font-semibold active:scale-[.97] ${a.primary ? "border-river bg-river text-ivory" : "border-line bg-white text-charcoal"}`}>
              <a.icon className="size-7" />{a.label}
            </Link>
          ))}
        </div>
      </section>
      ...
      <section className="mt-7 px-5">
        <div className="mb-3 flex items-baseline justify-between"><h2 className="text-xs font-medium uppercase tracking-[0.18em] text-muted">Today&apos;s tasks</h2><Link href="/site/tasks" className="text-sm font-medium text-river">See all</Link></div>
        <div className="space-y-3">
          {open.slice(0, 4).map((t) => <TaskCard key={t.id} t={t} showProject={projects.length > 1} />)}
          {open.length === 0 && <p className="rounded-2xl border border-dashed border-line bg-white p-5 text-center text-sm text-muted">{tasks.length ? "All caught up — nice work." : "No tasks assigned to you yet."}</p>}
        </div>
      </section>
    </>
  );
