    ...
    <>
      ...
      <div className="space-y-6 p-5">
        ...
        <div className="space-y-3">
          <Link href="/site/notifications" className={row}><Bell className="size-5 text-river" />{tr("Notifications")}{unread > 0 && <span className="rounded-full bg-laterite px-2 py-0.5 text-xs text-white">{unread}</span>}<ChevronRight className="ml-auto size-5 text-muted" /></Link>
          <Link href="/site/profile" className={row}><UserRound className="size-5 text-river" />{tr("Profile & password")}<ChevronRight className="ml-auto size-5 text-muted" /></Link>
          {u.role !== "CONTRACTOR" && <Link href="/site/attendance" className={row}><Users className="size-5 text-river" />{tr("Attendance")}<ChevronRight className="ml-auto size-5 text-muted" /></Link>}
          {u.role !== "CONTRACTOR" && <Link href="/site/materials" className={row}><Package className="size-5 text-river" />{tr("Materials")}<ChevronRight className="ml-auto size-5 text-muted" /></Link>}
        ...
        <div><h2 className="mb-3 text-xs font-medium uppercase tracking-[0.18em] text-muted">{tr("My projects")}</h2>
        ...
        <SignOut className={`${row} w-full text-red-700`}><LogOut className="size-5" />{tr("Sign out")}</SignOut>
      </div>
    </>
  );
}
