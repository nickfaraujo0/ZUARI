  ...
  return (
    <>
      ...
      <div className="space-y-5 p-5">
        <div>
          <div className="flex flex-wrap gap-2"><Chip tone={TASK_TONE[t.status]}>{TASK_STATUS[t.status]}</Chip><Chip tone={PRIORITY_TONE[t.priority]}>{PRIORITY[t.priority]} priority</Chip></div>
        ...
        <div className="rounded-2xl border border-line bg-white p-4">
          ...
          <p className="mt-3 text-sm text-muted">{dueLabel(t.dueDate)}{t.dueDate ? ` · ${fmtDate(t.dueDate)}` : ""}</p>
        </div>
        {t.description && <p className="whitespace-pre-line text-[15px] leading-relaxed text-charcoal/85">{t.description}</p>}
        <Link href={`/site/add-progress?project=${t.projectId}&task=${t.id}`} className="flex h-14 items-center justify-center gap-2.5 rounded-2xl bg-river text-base font-semibold text-ivory active:scale-[.98]"><Camera className="size-5" />Update this task</Link>
        {updates.length > 0 && (
          <div>
            <h3 className="mb-3 text-xs font-medium uppercase tracking-[0.18em] text-muted">History</h3>
