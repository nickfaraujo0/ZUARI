...
export default async function SiteLayout({ children }: { children: React.ReactNode }) {
  ...
  if (u.role === "ACCOUNTANT") redirect("/dashboard"); // finance role: no site app
  ...
  return (
    <div className="min-h-dvh bg-stone-200/50">
      <div className="relative mx-auto min-h-dvh max-w-[480px] bg-ivory pb-28 shadow-xl"><I18nProvider locale={u.locale}>{children}<RegisterSW /><SyncStatus userId={u.id} /><SiteNav unread={unread} /></I18nProvider></div>
    </div>
  );
}
