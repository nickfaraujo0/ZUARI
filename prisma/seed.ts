...
const PEOPLE: Record<string, { name: string; role: Role; title: string; phone: string }> = {
...
const PROJECTS: { name: string; client: string; location: string; type: "RESIDENTIAL" | "COMMERCIAL"; budget: number; start: number; end: number; manager: string; team: string[]; description: string; phases: Ph[]; photos: Photo[]; issues: Issue[]; reports: Report[] }[] = [
  {
    name: "Dona Paula Villa", client: "Mr. & Mrs. Kamat", location: "Dona Paula, Goa", type: "RESIDENTIAL", budget: 32_000_000, start: -300, end: 75, manager: "priya", team: ["carlos", "joao"],
    ...
    phases: [
      ...
      ["Electrical", -90, 10, [["Conduit laying — ground floor", 100, "carlos", null], ["Conduit laying — first floor", 100, "carlos", null], ["Electrical Installation", 50, "carlos", 0, "HIGH"], ["DB and main panel installation", 38, "carlos", 3]]],
    ...
    issues: [
      { title: "Water leakage — Floor 2", area: "First floor · Master bathroom", description: "Damp patch on the ceiling below the master bathroom slab.", severity: "MEDIUM", status: "ASSIGNED", by: "carlos", to: "priya", daysAgo: 0, h: 9, due: 3, scene: 4 },
  ...
  {
    name: "Candolim Residence", client: "The Almeida Family", location: "Candolim, Goa", type: "RESIDENTIAL", budget: 58_000_000, start: -220, end: 160, manager: "priya", team: ["anita", "joao"],
  ...
  },
  {
    ...
    issues: [
      { title: "Waterlogging near material storage", area: "Site yard · North", description: "Rain water pooling around cement store; bags at risk.", severity: "HIGH", status: "IN_PROGRESS", by: "joao", to: "rohan", daysAgo: 2, h: 11, due: 1, scene: 1 },
      { title: "Rebar spacing deviation — GF columns C4, C5", area: "Ground floor · Grid C", description: "Stirrup spacing measured wider than drawing; needs engineer review before further casting.", severity: "CRITICAL", status: "OPEN", by: "joao", daysAgo: 0, h: 8, due: 1, scene: 1 },
    ],
...
async function main() {
  ...
  for (const [k, p] of Object.entries(PEOPLE)) U[k] = (await prisma.user.create({ data: { companyId: co.id, email: email(k), name: p.name, role: p.role, title: p.title, phone: p.phone, passwordHash: hash } })).id;
