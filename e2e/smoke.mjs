...
for (const [path, o] of [["/inventory", { text: "Where everything is" }], ["/inventory/manage", {}], ["/inventory/transfers", {}]]) await visit(dir, path, o);
await dir.p.goto(`${BASE}/inventory/transfers`); await visit(dir, await dir.p.locator('a[href^="/inventory/transfers/c"]').first().getAttribute("href"), { text: "Date of transport" });
await dir.p.goto(`${BASE}/inventory`); await visit(dir, await dir.p.locator('a[href^="/inventory/items/"]').first().getAttribute("href"), { text: "Movement history" });
await visit(dir, await dir.p.locator('a[href^="/inventory/at/w-"]').first().getAttribute("href"), { text: "Stock here now" });
await visit(dir, `/inventory/at/p-${dona.split("/").pop()}`, { text: "Movements at this location" });
await visit(dir, "/projects/does-not-exist", { expect: 404 });
const pm = await session("priya.naik@coastalindia.demo");
for (const path of ["/dashboard", "/projects", "/tasks", "/team", "/profile", "/workforce", "/materials", "/procurement", "/documents", "/analytics", "/reports", "/calendar", "/timeline", "/contractors", "/suppliers", "/inspections", "/safety", "/bills", "/exports", "/ask", "/inventory", "/inventory/transfers"]) await visit(pm, path);
...
for (const path of ["/dashboard", "/projects", "/budget", "/expenses", "/payments", "/procurement", "/documents", "/analytics", "/contractors", "/suppliers", "/calendar", "/map", "/profile", "/bills", "/exports", "/ask", "/inventory"]) await visit(acc, path);
for (const path of ["/workforce", "/materials", "/tasks", "/photos", "/issues", "/team", "/reports", "/settings", "/inspections", "/safety", "/inventory/manage"]) await visit(acc, path, { expect: 404 });
...
for (const path of ["/site/attendance", "/site/materials", "/site/drawings", "/site/inspections", "/site/safety", "/site/delay", "/site/inventory"]) await visit(sup, path);
await visit(sup, "/documents", { expect: "redirect" }); await visit(sup, "/inventory", { expect: "redirect" }); await visit(sup, "/budget", { expect: "redirect" });
for (const who of ["vikram.shetty", "suresh.fernandes"]) {
  ...
  if (who.startsWith("vikram")) for (const path of ["/site/attendance", "/site/materials", "/site/inspections", "/site/inventory"]) await visit(s2, path);
  else { await visit(s2, "/site/materials", { expect: "redirect" }); await visit(s2, "/site/inspections", { expect: "redirect" }); await visit(s2, "/site/inventory", { expect: "redirect" }); await visit(s2, "/site/safety", {}); }
